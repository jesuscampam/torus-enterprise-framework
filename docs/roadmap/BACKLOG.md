...
| Dependencias | **Fijar `starlette` con `==` en `requirements.txt`.** Hoy entra como transitiva de FastAPI, cuyo constraint (`>=0.46.0`) no tiene techo: una instalación limpia en 3.14 trajo `1.6.0` mientras el entorno de desarrollo tiene `1.4.1`. Contradice la política de versiones fijadas de [DEPENDENCIES.md](../DEPENDENCIES.md). Detectado en Sprint 3.0.3, anterior a él | 🟡 | — |
| Rendimiento | **Revisar la baseline de benchmarks en una máquina estable.** En Sprint 3.0.3 la puerta `benchmarks` quedó en rojo con 6 mediciones entre +64 % y +80 %. Descartado que lo cause el cambio del sprint mediante un experimento controlado (mismos benchmarks con el pin anterior de `pydantic` → misma degradación, al microsegundo). Es el anfitrión. **La baseline no se regeneró a propósito** — ver [BENCHMARKS.md](../BENCHMARKS.md) | 🟡 | Una máquina dedicada |
