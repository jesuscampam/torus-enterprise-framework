...
**TypeScript 5.9**: se fija la línea 5.x y no la 7.x (port nativo) porque `typescript-eslint` construye sus reglas *type-aware* sobre la API programática del compilador, y TypeScript 7.0 se publicó sin esa API estable — no es cuestión de que el toolchain "aún no declare soporte", sino de que no puede hasta que la API exista en la 7.1. Igual de fijada queda Vitest en la línea 4.x frente a la 5.x, aquí por un suelo de Node (`engines`) que el frontend no ha subido todavía. Ambas condiciones de reapertura, evaluadas y verificadas, en [ADR-014](adr/ADR-014-typescript-7-vitest-5-deferred.md).
