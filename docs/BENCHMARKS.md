...
## Estado en Sprint 3.0.3 — la puerta está en rojo, y no por el código

En la validación de v0.10.3-alpha, seis mediciones salieron entre **+64 % y +80 %** sobre esta
baseline: `resolve()` en sus tres *lifetimes*, `publish()` con un suscriptor, `subscribe()` +
`unsubscribe()`, búsqueda de capacidad y compresión GZip.

**No lo causa el cambio del sprint** (subir `pydantic`, `asyncpg` y `sqlalchemy`). Se comprobó con
un experimento controlado: los mismos benchmarks, en la misma máquina y en la misma sesión, con el
**pin anterior `pydantic==2.10.4`**, dan la misma degradación — GZip midió 2048.70 µs con 2.10.4 y
2048.12 µs con 2.12.0, el mismo número. Y lo que se degrada incluye compresión GZip y resolución
del contenedor de DI, que no tocan `pydantic` por ningún camino. La explicación que queda es la que
esta misma página ya advierte dos secciones más arriba: **el anfitrión es más lento que cuando se
registró la baseline**.

La baseline **no se ha regenerado**. Reescribirla habría puesto la puerta en verde sin que nadie
entienda nada, que es precisamente lo que la sección siguiente prohíbe. Queda en rojo, declarada
aquí y en el [CHANGELOG](../CHANGELOG.md), para revisarla en una máquina estable.
