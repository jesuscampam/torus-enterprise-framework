...
A diferencia de ``backend/runtime/`` (que nunca depende de
``backend/contracts/`` ni ``backend/providers/``), el SDK **sí** depende de
``backend/core/`` y ``backend/runtime/`` — es la capa que se apoya en ambos
...
#: (``backend/core/application.py``). Un módulo declara compatibilidad con
